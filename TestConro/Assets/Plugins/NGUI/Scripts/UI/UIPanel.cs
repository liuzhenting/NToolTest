...
public class UIPanel : UIRect
{
	...
	// --==GAM==--
	// todo: rename mSortingOrderOrigin to mSortingOrder
	//[UnityEngine.Serialization.FormerlySerializedAs("mSortingOrderOrigin")]
	//[HideInInspector][SerializeField] int mSortingOrder = 0;
	[HideInInspector][SerializeField] int mSortingOrderOrigin = 0;
	// --==GAM==--
	...
	public int sortingOrder
	{
		get
		{
			return mSortingOrderOrigin;
		}
		set
		{
			if (mSortingOrderOrigin != value)
			{
				mSortingOrderOrigin = value;
#if UNITY_EDITOR
				NGUITools.SetDirty(this);
#endif
				UpdateDrawCalls();
	...
	void UpdateDrawCalls ()
	{
		...
		for (int i = 0; i < drawCalls.Count; ++i)
		{
			...
			dc.sortingOrder = mSortingOrderOrigin;
