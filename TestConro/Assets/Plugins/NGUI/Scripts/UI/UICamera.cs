...
public class UICamera : MonoBehaviour
{
	...
	static private RaycastHit[] hitsBuffer = new RaycastHit[MaxColliderOverlap];
	static private int hitsBufferLength = 0;
	// --==GAM==--
	static public bool Raycast (Vector3 inPos)
	{
		for (int i = 0; i < list.size; ++i)
		{
			...
			{
				// --==GAM==--
				hitsBufferLength = Physics.RaycastNonAlloc(ray, hitsBuffer, dist, mask);
				// check overflow
				while (hitsBufferLength == hitsBuffer.Length)
				{
					hitsBuffer = new RaycastHit[hitsBufferLength * 2];
					hitsBufferLength = Physics.RaycastNonAlloc(ray, hitsBuffer, dist, mask);
				}
				RaycastHit[] hits = hitsBuffer;

				if (hitsBufferLength > 1)
				{
					for (int b = 0; b < hitsBufferLength; ++b)
					{
				// --==GAM==--
